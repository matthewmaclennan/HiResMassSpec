...
#violated, the current loop is null and the next loop begins.
              ...
              if((k+m+i+l)<abs(j) | (k+m+i+l)>(2*(k+m+i+l)+j)){is.null(k)} else {
                ...
                a<-12.000000*k+1.007825*(2*k+2-j+m)+15.994915*i+14.003074*m+31.972072*l
                if(a<massmax & a>massmin) d<-rbind(d,cbind(cbind(a,-j,k,2*k+2-j+m,i,m,l,charge),
                (a+charge*(1.007825-0.0005))/cbind(a,-j,k,2*k+2-j+m,i,m,l,charge)[,8]))
              }
          }
        }
      }
  ...
  colnames(d)<-c("Neutral Mass","Z-number","C","H","O","N","S","Charge","m/z")
  d<-d[-1,]
}


nagenS0N0n1<-nagenmzhr(5,35,6,8,0,0,100,600,-2,-1)
nagenS0N6n1<-nagenmzhr(5,35,6,8,0,6,100,600,-2,-1)

nagenC18neg[nagenC18neg[,"C"]==18 & nagenC18neg[,"H"]==36,]

nagenS0N0n1[(round(abs(nagenS0N0n1[,"m/z"]),4)-223.1703)/223.1703*1e6<=1.5 & (round(abs(nagenS0N0n1[,"m/z"]),4)-223.1703)/223.1703*1e6>=(-1.5),]
